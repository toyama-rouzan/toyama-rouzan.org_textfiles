...
#┌─────────────────────────────────
#│ JOYFUL NOTE : regist.cgi - 2013/04/07
#│ Copyright (c) KentWeb
...
#-----------------------------------------------------------
sub regist {
	#LINE通知ライブラリ 2021/07/05 yoshioka
	require './lib/lineapi.pl' if ($cf{line_notify});
	...
	#LINEにメッセージを送信 2021/07/05 yoshioka
	if($cf{line_notify}){
		if($in{line_token1}) {
			if($cf{line_token1}){
				my @comment = split(/<br>/,$in{comment});
				line_push_message($cf{line_token1},"\n[投稿者] $in{name}\n[タイトル] $in{sub}\n[最初の行] $comment[0]");
			}
			if($cf{line_groupid1}){
				my @comment = split(/<br>/,$in{comment});
				my $LineMessagingAPIStatus = line_BusinessAccount_push_message($cf{line_business_account_access_token},
													$cf{line_groupid1},
													"=== 情報交換掲示板 ===\n【投稿者】$in{name}\n【タイトル】$in{sub}\n【1行目】$comment[0]");
				if($LineMessagingAPIStatus != 200){
					$AddtionalMessage = 'LINEへの通知はできませんでした';
					if($LineMessagingAPIStatus == 429){
						$AddtionalMessage = "$AddtionalMessage(今月の無料通知の上限を超えました。LINEアプリから手動でメッセージを送ってください)";
					}else{
						$AddtionalMessage = "$AddtionalMessage($LineMessagingAPIStatus)";
					}
				}
			}
		}
	...
	# 完了画面
	my $Messages="ありがとうございます。記事を受理しました。";
	if($AddtionalMessage ne ''){
		$Messages .= "<BR>$AddtionalMessage";
	}
