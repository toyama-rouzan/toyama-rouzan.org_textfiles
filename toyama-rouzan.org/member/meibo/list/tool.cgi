...
<BR>
<table><tr><td>
<span class="bold">▼$namaeの追加</span><BR>
※$namaeと初期パスワードを入力して「$namaeの追加」をクリックしてください。<br>
※途中に挿入すると、それ以降の番号が自動的に1ずつ加算されます。
<form action="$cgipath" method="$method">
<INPUT type="hidden" name="mpass" value="$m_pass">
<INPUT type="hidden" name="mode" value="add">
<TR><TD ALIGN="center">$namae：<INPUT name="name" size="15">　初期パスワード：<input type=password name="pass" size=8 maxlength=8> 
EOM
...
print "</select> &nbsp;";
print <<EOM;
<INPUT type="submit" value=" 追加 "></TD></TR>
</TABLE>
</form>
<form action="$cgipath" method="$method">
...
<FORM action="$cgipath" method="$method">
<span class="bold">▼$namaeの削除</span><BR>
<SMALL>※完全削除をするとその人以降の番号が自動的につまります（再登録不可）</SMALL><BR>
<SMALL>※欠番処理の場合は、削除前のパスワードまたは管理パスで$namaeを再登録できます。</SMALL><BR>
<INPUT type="hidden" name="mode" value="del">
<INPUT type="hidden" name="mpass" value="$m_pass">
削除するNo.：<INPUT name="id" size="3">　
処理：<select name=deltype>
<option value=1>完全削除
<option value=2>欠番処理
</select>
 &nbsp;<INPUT type="submit" value="$namaeの削除">
</FORM>
...
<HR WIDTH="100%">
<FORM action="$cgipath" method="$method">
<span class="bold">▼index.htmlの作成</span><BR>
<SMALL>※index.htmlがうまく作成されなかった場合や、更新をしたい時に使ってください。</SMALL><BR>
<INPUT type="hidden" name="mode" value="make">
<INPUT type="hidden" name="mpass" value="$m_pass">
<INPUT type="submit" value="index.htmlの作成">
</FORM>

<FORM action="$cgipath" method="$method">
<span class="bold">▼名簿の最適化（番号のふり直し）</span><BR>
<SMALL>※欠番状態のものを全て削除し、番号を小さい順からふり直します。</SMALL><BR>
<SMALL>※削除や欠番によって飛んだ番号をふり直すので、<b>登録者の番号が変わってもいい場合</b>のみ行ってください。</SMALL><BR>
<SMALL>※また、<b>トップソ\ートのON-OFFを切り替えた時</b>にも、1度最適化することをおすすめします。</SMALL><BR>
<INPUT type="hidden" name="mode" value="remake">
<INPUT type="hidden" name="mpass" value="$m_pass">
<INPUT type="submit" value="名簿の最適化">
</FORM>

<FORM action="$cgipath" method="$method">
<span class="bold">▼ログファイルを出力（＆バックアップ）</span><BR>
<SMALL>※<b>そのまま出力 </b> ： データのバックアップを取るならこれ。$datapathをダウンロードします。</SMALL><BR>
<SMALL>※<b>XML形式で出力</b>(おまけ) ： <b>名前と項目のみ</b>をXML形式で出力。拡張子は<b>.xml</b>にします。</SMALL><BR>
<SMALL>※<b>CSV形式で出力</b>(おまけ) ： <b>名前と項目のみ</b>をCSV形式で出力。拡張子は<b>.csv</b>にします</SMALL><BR>
<select name=type>
<option value=backup>そのまま出力（バックアップ）
<option value=xml>XML形式に変換して出力
<option value=csv>CSV形式に変換して出力
</select>
